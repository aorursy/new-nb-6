...
train.head()
pprint.pprint(train.iloc[0]['cast'])
...
for i, p in enumerate([j[0] for j in d]):
    ...
    try:
        ...
        print(f"Image for {p[0]} not found on profile_path.")
...
layout = go.Layout(dict(title = "Number of films and average revenue per year",
                ...
                orientation="v"))
py.iplot(dict(data=data, layout=layout))
sns.stripplot(x='release_date_weekday', y='revenue', data=train);
...
sns.stripplot(x='num_genres', y='revenue', data=train);
...
for i, e in enumerate([col for col in train.columns if 'production_company' in col]):
    sns.violinplot(x=e, y='revenue', data=train, ax=axes[i // 5][i % 5]);
sns.stripplot(x='num_countries', y='revenue', data=train);
