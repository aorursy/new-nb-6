...
def prepare_features(df, gap):
    ...
    df["change_1_5_cc"] = df[f"lag_{gap}_cc"] / df[f"lag_{gap + 5}_cc"]

    df["change_1_5_ft"] = df[f"lag_{gap}_ft"] / df[f"lag_{gap + 5}_ft"]
    ...
    df["change_1_10_cc"] = df[f"lag_{gap}_cc"] / df[f"lag_{gap + 10}_cc"]

    df["change_1_10_ft"] = df[f"lag_{gap}_ft"] / df[f"lag_{gap + 10}_ft"]
...
def predict_mad(df_test, gap, val = False):
    ...
    if val:
        ...
        y_pred_cc = df_test[f"lag_{gap}_cc"] + gap * df_test.avg_diff_cc - (1 - MAD_FACTOR_CC) * df_test.avg_diff_cc * np.sum([x for x in range(gap)]) / VAL_DAYS

        y_pred_ft = df_test[f"lag_{gap}_ft"] + gap * df_test.avg_diff_ft - (1 - MAD_FACTOR_FT) * df_test.avg_diff_ft * np.sum([x for x in range(gap)]) / VAL_DAYS

    else:

        y_pred_cc = df_test[f"lag_{gap}_cc"] + gap * df_test.avg_diff_cc - (1 - MAD_FACTOR_CC) * df_test.avg_diff_cc * np.sum([x for x in range(gap)]) / n_dates_test

        y_pred_ft = df_test[f"lag_{gap}_ft"] + gap * df_test.avg_diff_ft - (1 - MAD_FACTOR_FT) * df_test.avg_diff_ft * np.sum([x for x in range(gap)]) / n_dates_test
...
df_test["ConfirmedCases"] = 0.47 * df_test.ConfirmedCases_test_lgb + 0.53 * df_test.ConfirmedCases_test_mad

df_test["Fatalities"] = 0.21 * df_test.Fatalities_test_lgb + 0.79 * df_test.Fatalities_test_mad
