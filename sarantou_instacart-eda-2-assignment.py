...
sns.barplot(data = aisle_top_sort, x=aisle_top_sort.index, y='total_products', order=aisle_top_sort.index, palette="Blues_d" )
plt.xlabel('Aisle IDs', size=15)
plt.ylabel('Products per Aisle ID', size=15)
# Modify the limits
plt.ylim(0,1300)
plt.show()
...
sns.barplot(x=avg_pos_asc.index[0:10], data=avg_pos_asc.iloc[0:10],  y='mean_add_to_cart_order' , order=avg_pos_asc.index[0:10], palette='Reds_d')
plt.xlabel('Product_IDs', size=15)
plt.ylabel('Means of Products', size=15)
plt.ylim(0,2.6)
plt.show()
